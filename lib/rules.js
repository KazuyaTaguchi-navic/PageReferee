...
(function (global) {
  ...
  function extractFitmentWindow(html) {
    ...
    const m = raw.match(FITMENT_HEADING_RE);
    if (!m) return "";
  ...
  // セット商品（例: "sankei-se-1751rl-set"）は、管理表の「品番」「SKU」列に構成する単品側の
  // 値を「+」で連結して追記していく運用になっている（例: "SE-1751L+SE-1751R"）。
  // このため通常商品と同じ完全一致比較をそのまま適用すると、以下の運用上正しい入力が
  // 誤って不一致（イエローカード）として検出されてしまう（2026年8月のページ作成担当者からの
  // フィードバックより）:
  //  - ヤフーJANコード／Yahoo製品コード: セット商品は空欄が正解（実際にはこれまで未入力の運用）
  //  - 楽天カタログID: セット商品は連結値そのものではなく、構成する単品側(通常はL側)のSKUを
  //    入力する運用
  const SET_COMBINED_VALUE_RE = /\+/;
  const SET_BLANK_ALLOWED_KEYS = new Set(["ヤフーJANコード", "Yahoo製品コード"]);

  // equalityPairsの1件について、セット商品向けの許容パターンに当てはまるため
  // 不一致として報告すべきでないかどうかを判定する
  function isSetEqualityException(domKey, actual, expected) {
    if (!SET_COMBINED_VALUE_RE.test(String(expected))) return false;
    if (SET_BLANK_ALLOWED_KEYS.has(domKey)) {
      return isZeroOrEmpty(actual);
    }
    if (domKey === "楽天カタログID") {
      const parts = String(expected)
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      return parts.some((part) => valuesEqual(actual, part));
    }
    return false;
  }
  ...
  function runChecks(params) {
    ...
    if (managementMatch) {
      for (const pair of equalityPairs || []) {
        ...
        if (!valuesEqual(actual, expected) && !isSetEqualityException(pair.domKey, actual, expected)) {
          addFinding(
            pair.domKey,
    ...
    // 実行できる）。
    {
      ...
      for (const target of pcMobileTargets) {
        ...
        const onlyPc = Array.from(pcMap.keys())
          .filter((k) => !mobileMap.has(k))
          .map((k) => pcMap.get(k));
        const onlyMobile = Array.from(mobileMap.keys())
          .filter((k) => !pcMap.has(k))
