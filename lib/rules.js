  ...
  function runChecks(params) {
    ...
    if (managementMatch) {
      ...
      if (modelValue && String(modelValue).trim()) {
        ...
        // 商品名・商品名略称自体は、楽天商品名／Yahoo商品名が正しく更新されていても、下書き元の
        // #product_name／informality_nameのコピー元の内容（車種・商品カテゴリ等）が残ったまま
        // 見落とされることがあるため、フォールバック用途に留めず常に独立してチェックする
        // （ページ作成担当者からのフィードバックより）。
        const modelTargets = [
          ...
          { key: "商品名", nameKeys: ["商品名"], label: "商品名", freeText: allFreeText },
          { key: "商品名略称", nameKeys: ["商品名略称"], label: "商品名略称", freeText: allFreeText },
        ];
    ...
    if (anyAsinFilled) {
      ...
      if (!isZeroOrEmpty(mgmtSaleTaxValue)) {
        const mgmtSaleTaxDisplay = formatAmountForDisplay(mgmtSaleTaxValue);
        if (isZeroOrEmpty(amazonPrice)) {
          addFinding(
            "amazon_price_missing",
            `Amazonの「販売価格（税込）」が空欄です（管理表の売価税込「${mgmtSaleTaxDisplay}」を入力してください）`,
            "red",
            { copyValue: mgmtSaleTaxDisplay },
            "Amazon販売価格"
          );
        } else if (!valuesEqual(amazonPrice, mgmtSaleTaxValue)) {
          addFinding(
            "amazon_price_mismatch",
            `Amazonの「販売価格（税込）」が管理表の売価税込と一致しません（Amazon: ${amazonPrice} / 管理表: ${mgmtSaleTaxDisplay}）`,
            "red",
            { copyValue: mgmtSaleTaxDisplay },
            "Amazon販売価格"
          );
      ...
      // 3.配送パターン: 税込売価が3980円未満（送料別）なら「基本設定」、
      // 3980円以上（送料込み）なら「マケプレお急ぎ便配送パターン」にする必要がある
      // （ページ作成担当者からのフィードバックより。楽天/Yahoo!の送料区分と同じ基準）。
      const amazonShippingPattern = domValues["Amazon配送パターン"];
      if (isZeroOrEmpty(amazonShippingPattern)) {
        addFinding(
      ...
      } else if (!isZeroOrEmpty(mgmtSaleTaxValue)) {
        const isFreeShipping = Number(normalizeForCompare(mgmtSaleTaxValue)) >= 3980;
        const expectedPattern = isFreeShipping ? "マケプレお急ぎ便配送パターン" : "基本設定";
        if (!valuesEqual(amazonShippingPattern, expectedPattern)) {
          addFinding(
            "amazon_shipping_pattern_mismatch",
            `管理表の売価税込「${formatAmountForDisplay(mgmtSaleTaxValue)}」円は「${isFreeShipping ? "3980円以上" : "3980円未満"}」のため、Amazonの「配送パターン」は「${expectedPattern}」にする必要があります（現在: ${amazonShippingPattern}）`,
            "red",
            {},
            "Amazon配送パターン"
          );
      ...
      // Amazonの「ポイント」は、ASINの有無を問わず常に空欄が必要なFIXED_RULE_CHECKS側の
      ...
      // Amazonの「法人価格」も、ASINの有無を問わず常に空欄が必要なFIXED_RULE_CHECKS側の
      // ルールで一括してチェックする（ここで重複してチェックすると同じ内容が2件出てしまう）。
