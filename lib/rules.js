    ...
    if (managementMatch) {
      for (const pair of equalityPairs || []) {
        ...
        if (isZeroOrEmpty(expected)) {
          // オープン価格等で管理表の定価税抜自体が空欄/0の場合、対応する
          // 「メーカー希望小売価格」がページ上でも空欄なのは正しい状態
          // （要確認の対象ではない）。3-1.楽天市場ページ作成マニュアルの[5]定価より。
          if (pair.domKey === "メーカー希望小売価格" && isZeroOrEmpty(actual)) {
            continue;
          }
          if (pair.isAmount) {
      ...
      // <BR>等が単語にくっつくと分割がずれるため、比較前にHTMLタグを取り除く。
      {
        ...
        if (rakutenTitle && yahooTitle) {
          ...
          const yahooFreeText = ((freeTextKeys && freeTextKeys.yahoo) || [])
            .map((k) => domValues[k] || "")
            .join(" ");
          const onlyInRakuten = findMissingWords(rakutenTitle, yahooTitle, yahooFreeText);
          const onlyInYahoo = findMissingWords(yahooTitle, rakutenTitle, rakutenFreeText);
          if (onlyInRakuten.length || onlyInYahoo.length) {
            addFinding(
              "rakuten_yahoo_name_diff",
              `楽天の商品名とYahoo!の商品名に差異があるようです。楽天にしかない語:「${onlyInRakuten.join("、") || "(なし)"}」／Yahoo!にしかない語:「${onlyInYahoo.join("、") || "(なし)"}」。入力ミス・コピペミスがないか確認してください（文字数の都合で検索ワード欄等に移した場合は問題ありません）`,
              "yellow",
              {},
              "楽天商品名"
            );
