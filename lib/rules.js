...
(function (global) {
  ...
  function runChecks(params) {
    ...
    if (managementMatch) {
      ...
      // ルールブックNo.6/17: 商品名が文字数（バイト数）制限を超える場合、型式・車種名等の
      // 優先度の低い情報は削除せず「■検索ワード」として商品説明欄に移してよいことになっている。
      // そのため1b/1cでは、商品名側だけでなく同じモールの説明文（検索ワードの置き場所）も
      // 検索範囲に含め、そちらに移されているだけの語（本当の更新漏れではない）を誤検知しない
      // ようにする。まとめて含まれているかだけでなく語単位で判定するのは、「ハイラックス
      // 30系 40系...150系」のような複数の型式のうち一部だけを検索ワード欄に移した場合でも
      // 対応できるようにするため。
      function findMissingWords(needle, mainText, freeText) {
        const words = normalizeText(needle).toLowerCase().split(/[\s　]+/).filter(Boolean);
        const searchSpace = normalizeText(mainText).toLowerCase() + " " + normalizeText(freeText).toLowerCase();
        return words.filter((w) => !searchSpace.includes(w));
      }
      ...
      if (modelValue && String(modelValue).trim()) {
        const modelTargets = [
          { key: "楽天商品名", label: "楽天の商品名", freeTextGroup: "rakuten" },
          { key: "Yahoo商品名", label: "Yahoo!の商品名", freeTextGroup: "yahoo" },
        ];
        for (const target of modelTargets) {
          const text = domValues[target.key] || "";
          if (!text) continue;
