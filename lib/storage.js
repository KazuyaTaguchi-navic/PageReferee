...
(function (global) {
  ...
  const EQUALITY_PAIRS = [
    ...
    // ヤフー定価は新規作成時点では常に空欄にする運用に変更されたため、管理表との
    // 突き合わせは行わず、FIXED_RULE_CHECKSで空欄チェックのみ行う。
  ...
  const FIXED_RULE_CHECKS = [
    ...
    {
      // 4-1.Yahoo!ショッピングページ作成マニュアル [5]定価。新規作成時点では常に
      // 空欄にする運用に変更された（ページ作成担当者からのフィードバックより）。
      domKey: "ヤフー定価",
      expectedValue: "",
      message: "Yahoo!の「定価」は新規作成時点では空欄にする必要があります",
    },
    {
...
- Yahoo!の「定価」「特価」「会員向け価格」「Yahoo!ショッピング製品コード」は新規作成時点では空欄にする。
